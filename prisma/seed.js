...
async function main() {
    ...
    const vendorsData = [
        ...
        { name: 'Umbrella Corp', email: 'bio@umbrella.com', paymentTerms: 7, status: client_1.VendorStatus.INACTIVE, contactPerson: 'Albert Wesker' },
    ];
    const vendors = [];
    for (const v of vendorsData) {
        const vendor = await prisma.vendor.upsert({
            ...
            create: {
                ...
                status: v.status || client_1.VendorStatus.ACTIVE,
            },
        });
        vendors.push(vendor);
    }
    ...
    const activeVendors = vendors.filter(v => v.status === client_1.VendorStatus.ACTIVE);
    ...
    const paymentMethods = [client_1.PaymentMethod.NEFT, client_1.PaymentMethod.UPI, client_1.PaymentMethod.CHEQUE, client_1.PaymentMethod.CASH, client_1.PaymentMethod.RTGS];
    for (let i = 0; i < 10; i++) {
        const po = pos[i];
        if (!po)
            continue;
        ...
        await prisma.purchaseOrder.update({
            where: { id: po.id },
            data: { status: client_1.POStatus.PARTIALLY_PAID }
        });
    }
